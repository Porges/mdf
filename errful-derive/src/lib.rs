...
use syn::{parse_macro_input, token::Enum, DeriveInput, Label, Type};
...
type RefData<'a> = ast::Data<&'a EnumVariant, &'a StructField>;

trait BasicOptions {
    fn exit_code(&self) -> Option<&u8>;
    fn url(&self) -> Option<&str>;
    fn code(&self) -> Option<&str>;
    fn severity(&self) -> Option<&syn::Path>;
}

impl BasicOptions for Opts {
    fn exit_code(&self) -> Option<&u8> {
...
impl BasicOptions for EnumVariant {
    fn exit_code(&self) -> Option<&u8> {
        self.exit_code.as_ref()
    }
...
pub fn derive_errful(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let res = move || -> Result<proc_macro::TokenStream, darling::Error> {
        ...
        let source_method = generate_source_function(&data)?;
        let labels_fn = generate_labels_function(&data)?;
...
fn generate_source_function(data: &RefData) -> darling::Result<TokenStream> {
        ...
        for (ix, field) in struct_data.fields.iter().enumerate() {
                    ...
                    .unwrap_or_default()
            {
                if is_optional(&field.ty) {
                    sources.push(quote! {
                        if let Some(source) = self.#field_name {
                            use std::borrow::Borrow;
                    ...
                    sources.push(quote! {
                        use std::borrow::Borrow;
                        return Some(self.#field_name.borrow());
                    });
                }
            }
        }
...
fn generate_labels_function(data: &RefData) -> darling::Result<Option<TokenStream>> {
